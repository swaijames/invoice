...
def invoices(request):
    ...
    return render(request, 'dash/invoice_list.html', context)
...
def clients(request):
    ...
    if request.method == 'GET':
        form = ClientForm()
        context['form'] = form
        return render(request, 'dash/clients.html', context)
...
def viewPDFInvoice(request, slug):
    ...
    # Get Client Settings
    p_settings = Settings.objects.get(clientName='ITSolutions Company')
...
def companySettings(request):
    company = Settings.objects.get(clientName='ITSolutions Company')
